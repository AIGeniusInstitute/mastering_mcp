    ...
    async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
        ...
        if self.tool_calling_method == 'raw':
            ...
            try:
                ...
                if 'current_state' in parsed_json and 'action' in parsed_json:
                    parsed = self.AgentOutput(**parsed_json)
                elif 'parameters' in parsed_json:
                    parameters = parsed_json['parameters']
                    if 'current_state' in parameters and 'action' in parameters:
                        parsed = self.AgentOutput(**parameters)

                print(f"L1096 get_next_action:\n\n{parsed}\n\n")
                ...
                self.logger.warning(f'L1102 Failed to parse model output: {output} {str(e)}')

                raise ValueError('L1102 Could not parse response.')
        ...
        if not parsed:
            ...
            try:
                ...
                print(f"L1167 get_next_action:\n\n{parsed}\n\n")

            except Exception as e:
                print(e)
                self.logger.warning(f'L1166 Failed to parse model output: {parsed_json} {str(e)}')
                raise ValueError('L1166 Could not parse response.')
