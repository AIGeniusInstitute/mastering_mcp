...
def extract_json_from_model_output(content: str) -> dict:
    ...
    print(f"Extract JSON from model output, handling both plain JSON and code-block-wrapped JSON:\n\n{content}\n\n")
    ...
    try:
        ...
        # fix: [browser_use.agent.message_manager.utils] Failed to parse model output: <{"name": "AgentOutput", "parameters": {"current_state": {"evaluation_previous_goal": "Success - The search query 'Model Context Protocol (MCP) development tools and technology stacks for AI' has been successfully input into the Google search bar (index 7), as confirmed by the current page elements and visual context showing the query in the search field.", "memory": "Task: Find 20 high-quality sources about 'Model Context Protocol (MCP) development tools and technology stacks for AI'. Progress: 0 sources found. Steps taken: Navigated to Google (https://www.google.com.hk/), input search query into search bar (index 7). Current step: 7/10. Current page: Google search page with query entered, ready for submission.", "next_goal": "Submit the search query by clicking the 'Google 搜索' (Google Search) submit button (index 10) to retrieve the search results page."}, "action": [{"click_element": {"index": 10}}]}} Expecting value: line 1 column 1 (char 0)
        # if content start with '<', remove '<'
        ...
        logger.warning(f'L66 Failed to parse model output: {content} {str(e)}')
