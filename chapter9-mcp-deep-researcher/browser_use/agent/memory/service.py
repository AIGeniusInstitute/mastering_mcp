...
class Memory:
	...
	def __init__(
		...
		if config is None:
			...
			# Use HuggingFace all-MiniLM-L6-v2 as the default embedder
			self.config.embedder_provider = 'huggingface'
			self.config.embedder_model = 'sentence-transformers/all-MiniLM-L6-v2' # Or 'sentence-transformers/all-MiniLM-L6-v2'
			self.config.embedder_dims = 384
			
			# Fallback or alternative configurations can be added here if needed
			# For example, to still allow OpenAI or other providers based on some condition or llm_class:
			# llm_class = llm.__class__.__name__
			# if llm_class == 'ChatOpenAI':
			# 	self.config.embedder_provider = 'openai'
			# 	self.config.embedder_model = os.getenv('OPENAI_EMBEDDING_MODEL', 'text-embedding-3-small')
			# 	self.config.embedder_dims = int(os.getenv('OPENAI_EMBEDDING_DIMS', 1536))
